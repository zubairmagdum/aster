...
export const T = {
  ...
  gold:"#8A6D00",gold2:"#D4B07A",
  charcoal:"#1C1C1C",gray:"#595959",gray2:"#6B6B6B",gray3:"#808080",
  white:"#FFFFFF",sage:"#4A7C59",sage2:"#A8C4A2",
...
export function checkHardSkip(jdText, prefs) {
  ...
  excludedDomains.forEach(domain => {
    const keywords = Object.prototype.hasOwnProperty.call(domainMap, domain) ? domainMap[domain] : [domain.toLowerCase()];
    if (Array.isArray(keywords) && keywords.some(k => jd.includes(k))) {
...
export function parseCSVData(csvText){
  ...
  return dataLines.map(line=>{
    ...
    if(company.toLowerCase().trim()==="company"||role.toLowerCase().trim()==="role")return null;
...
// ─── ROBUST JSON PARSER ──────────────────────────────────────────────────────
export function safeParseClaudeResponse(text) {
  if (!text || typeof text !== 'string') return { _parseError: true, raw: '' };
  ...
  // Attempt 3: extract first JSON array from text
...
// ─── DUPLICATE DETECTION ─────────────────────────────────────────────────────
export function checkDuplicate(jobs, comp, rl) {
  if (!comp) return null;
  return jobs.find(j => j.company?.toLowerCase() === comp?.toLowerCase() && j.role?.toLowerCase().includes((rl || "").toLowerCase().slice(0, 10))) || null;
}
