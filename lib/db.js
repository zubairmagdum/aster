...
export const dbSaveJob = async (job, userId) => {
  ...
  if (error) {
    console.error('[DB:dbSaveJob] ERROR details:', {
...
export const dbSaveAllJobs = async (jobs, userId) => {
  ...
  console.log('[DB:dbSaveAllJobs] raw job sample from localStorage:', JSON.stringify(jobs[0], null, 2));
  ...
  console.log('[DB:dbSaveAllJobs] mapped payload sample:', JSON.stringify(rows[0], null, 2));
  const { data, error } = await supabase.from('jobs').upsert(rows, { onConflict: 'user_id,local_id' });
  if (error) {
    console.error('[DB:dbSaveAllJobs] ERROR details:', {
      message: error.message,
      code: error.code,
      details: error.details,
      hint: error.hint,
...
export const dbDeleteJob = async (jobId, userId) => {
  ...
  if (error) console.error('[DB:dbDeleteJob] ERROR', error);
};
...
export const dbSaveResume = async (resumeText, fileName, userId) => {
  ...
  if (error) console.error('[DB:dbSaveResume] ERROR', error);
};
...
export const dbLoadResume = async (userId) => {
  ...
  if (error) { console.error('[DB:dbLoadResume] ERROR', error); return null; }
...
export const dbSavePrefs = async (prefs, userId) => {
  ...
  if (error) console.error('[DB:dbSavePrefs] ERROR', error);
};
...
export const dbLoadPrefs = async (userId) => {
  ...
  if (error) { console.error('[DB:dbLoadPrefs] ERROR', error); return null; }
...
export const dbSaveContact = async (contact, userId) => {
  ...
  if (error) console.error('[DB:dbSaveContact] ERROR', error);
};
...
export const dbEnsureUser = async (user) => {
  ...
  if (error) console.error('[DB:dbEnsureUser] ERROR', error);
};
