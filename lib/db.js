...
export const dbSaveJob = async (job, userId) => {
  ...
  try {
    ...
    return { error: e.message || 'Failed to save job' };
  }
};

export const dbSaveAllJobs = async (jobs, userId) => {
  if (!supabase || !userId || !jobs.length) return { error: null };
  ...
  try {
    ...
    return { error: e.message || 'Failed to save jobs' };
  }
};
...
export const dbLoadJobs = async (userId) => {
  ...
  try {
    ...
    console.error('dbLoadJobs failed:', e.message);
    return null;
  }
};
...
export const dbDeleteJob = async (jobId, userId) => {
  ...
  try {
    ...
    return { error: e.message || 'Failed to delete job' };
  }
};
...
export const dbSaveResume = async (resumeText, fileName, userId) => {
  if (!supabase || !userId) return { error: null };
  try {
    const { error } = await supabase.from('resumes').upsert({ user_id: userId, resume_text: resumeText, file_name: fileName }, { onConflict: 'user_id' });
    if (error) return { error: error.message };
    return { error: null };
  } catch (e) {
    return { error: e.message || 'Failed to save resume' };
  }
};
...
export const dbLoadResume = async (userId) => {
  ...
  try {
    ...
    console.error('dbLoadResume failed:', e.message);
    return null;
  }
};
...
export const dbSavePrefs = async (prefs, userId) => {
  if (!supabase || !userId) return { error: null };
  try {
    const { error } = await supabase.from('preferences').upsert({ user_id: userId, prefs }, { onConflict: 'user_id' });
    if (error) return { error: error.message };
    return { error: null };
  } catch (e) {
...
export const dbLoadPrefs = async (userId) => {
  ...
  try {
    ...
    console.error('dbLoadPrefs failed:', e.message);
    return null;
  }
};
...
export const dbLoadContacts = async (userId) => {
  ...
  try {
    ...
    console.error('dbLoadContacts failed:', e.message);
    return null;
  }
};
...
export const dbEnsureUser = async (user) => {
  ...
  try {
    const { error } = await supabase.from('users').upsert({ id: user.id, email: user.email }, { onConflict: 'id' });
    if (error) return { error: error.message };
    return { error: null };
  } catch (e) {
    return { error: e.message || 'Failed to ensure user' };
  }
};
...
export const dbSaveStrategy = async (strategy, userId) => {
  ...
  try {
    ...
    return { error: e.message || 'Failed to save strategy' };
  }
};
...
export const dbLoadStrategy = async (userId) => {
  ...
  try {
    ...
    console.error('dbLoadStrategy failed:', e.message);
    return null;
  }
};
...
export const dbSubscribeEmail = async (email, source = 'website', digestOptIn = false) => {
  ...
  try {
    // Check for existing subscriber to prevent duplicates
    const { data: existing } = await supabase.from('email_subscribers').select('email').eq('email', email).maybeSingle();
    if (existing) return { error: null }; // Already subscribed, no-op
    const { error } = await supabase.from('email_subscribers').insert({ email, source, digest_opt_in: digestOptIn });
    if (error) return { error: error.message };
    return { error: null };
  } catch (e) {
...
// ─── Feedback & Ratings ──────────────────────────────────────────────────────
// TODO(RLS): analysis_ratings and feedback INSERT policies are currently WITH CHECK (true).
// They should validate user_id = auth.uid() OR user_id IS NULL (for anonymous submissions).
// See supabase/migrations/010_security_fixes.sql for the fix.
export const dbSubmitRating = async (payload) => {
  if (!supabase) return { error: null };
  try {
    const { error } = await supabase.from('analysis_ratings').insert(payload);
    if (error) return { error: error.message };
    return { error: null };
  } catch (e) {
    return { error: e.message || 'Failed to submit rating' };
  }
};
...
export const dbSubmitFeedback = async (payload) => {
  ...
  try {
    ...
    return { error: e.message || 'Failed to submit feedback' };
  }
};
